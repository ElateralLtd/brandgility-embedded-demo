...
const app = {
  ...
  handleOpenTemplate(event) {
    ...
    const url = `${env.brandgilityUrl}/embedded-template-configure/new/${templateId}?action=START_TEMPLATE_CONFIGURATION&eid=${templateId}`;
  ...
  handleOpenSavedItem(event) {
    ...
    const url = `${env.brandgilityUrl}/embedded-template-configure/edit/${savedItemId}?action=SAVED_ITEM&eid=${savedItemId}`;
